...
<body>
	...
	<c:if test="${isName && isAge }">
		...
		<c:when test="${param.age >= 20 }">
			<h1 style="color:blue;">${isName }님 성인 인증되셨습니다</h1>
		</c:when>
		<c:when test="${param.age >= 17 && age <= 19}">
			<h1 style="color:yellow;">${isName }님은 고등학생입니다</h1>
		</c:when>
		<c:when test="${param.age >= 1 }">
			<h1 style="color:green;">${isName }님은 미성년입니다</h1>
		</c:when>	
		<c:otherwise>
